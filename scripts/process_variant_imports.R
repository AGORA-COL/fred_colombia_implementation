...
covid_file = sprintf('../data/Covid_Colombia_cases_%s.csv',date_format)
reports_covid_df = read_csv(covid_file) %>%
    ...
    mutate(DateDiagnosis = as.Date(DateDiagnosis),
            DateDeath = as.Date(DateDeath),
            DateReport = as.Date(DateReport),
            SymptomOnset = as.Date(SymptomOnset))
...
write.csv(variants_imports, '../input_files/COL_variant_imports.csv')
