...
## local variables
####################################################
workers = c("worker4", "worker5", "worker7", "worker8", "worker9", "worker12")
...
write_cmd_function <- function(scalars_in, tmpfile){
  ...
  job_cmd_str = sprintf("fred_job -f -k %s -I %d -p %s -n %.0f -t 5; Rscript ./post_process_fred_projection_lineages_BQX.R %s %.0f",
                        scalars_in$job_id,
                        scalars_in$run_id,
...
fred_defaults        = sprintf("%s/input_files/defaults", fred_home)
experiment_label     = sprintf('FRED_%.0f_projections_asymp_%.2f_fm_%.2f_ksus_%.2f_var_%.0f_vax_%03d_mov_%s',state_code, asymp_infectivity_in, face_mask_transmission_efficacy_in, kids_susceptibility_age_in, variants_in, vaccination_in, projection_label)
output.dir           = file.path(scratch_dir, experiment_label)
...
### Omicron BQX
scalars_sobol_df$variantomicronBQX_susceptibility_by_age_offset  = scalars_sobol_df$influenza_susceptibility_by_age_offset
scalars_sobol_df$variantomicronBQX_susceptibility_by_age_rate = scalars_sobol_df$influenza_susceptibility_by_age_rate 
scalars_sobol_df$variantomicronBQX_susceptibility_by_age_cutoff = scalars_sobol_df$influenza_susceptibility_by_age_cutoff
scalars_sobol_df$variantomicronBQX_susceptibility_by_age_high = scalars_sobol_df$influenza_susceptibility_by_age_high

scalars_sobol_df$variantomicronBQX_transmissibility_factor = 2.1
scalars_sobol_df$variantomicronBQX_transmissibility = scalars_sobol_df$variantomicronBQX_transmissibility_factor * scalars_sobol_df$influenza_transmissibility
scalars_sobol_df$variantomicronBQX_cross_protection_prob = 0.7 
scalars_sobol_df$variantomicronBQX_imports_factor = 2
...
primary_cases_normal_omicron_lineage_file_in = file.path(output.dir, sprintf('initial_cases_normal_omicron_lineage_%d_%d.txt',state_code,1:reps))
...
for(nn in 1:reps){
  ...
  if(variants_in == 1){
    ...
    ########################################################################################################
    omicronBQX_imports = filter(variants_imp_df, variant == "22A (Omicron)") %>%
      mutate(day = as.numeric(Date - as.Date('2020-01-01'))) %>%
      mutate(Imports = round(50*scalars_sobol_df$variantomicronBQX_imports_factor[nn] * TotalVariantImports)) %>%
      #filter(Imports >= 1, Date > as.Date('2021-12-15'), Date <= as.Date('2022-03-20')) %>%
      filter(Imports >= 1, Date > as.Date('2022-01-01')) %>%
      mutate(day = day + 40) %>%
      dplyr::select(day, Imports)
    
    # max_omicron = omicronBQX_imports %>% filter(day == max(omicronBQX_imports$day))
    # max_omicron = max_omicron[rep(1:nrow(max_omicron), numDays - max(max_omicron$day)),]
    # max_omicron$day = max_omicron$day[1] + 1:nrow(max_omicron)
    # omicronBQX_imports = bind_rows(omicronBQX_imports, max_omicron)
    ########################################################################################################
    plus_omicronBQX_imports = filter(variants_imp_df, variant == "22A (Omicron)") %>%
      mutate(day = as.numeric(Date - as.Date('2020-01-01'))) %>%
      mutate(Imports = round(50*scalars_sobol_df$variantomicronBQX_imports_factor[nn] * TotalVariantImports)) %>%
      #filter(Imports >= 1, Date > as.Date('2021-12-15'), Date <= as.Date('2022-03-20')) %>%
      filter(Imports >= 1, Date > as.Date('2022-01-01')) %>%
    ...
    # max_omicron = plus_omicronBQX_imports %>% filter(day == max(plus_omicronBQX_imports$day))
    # max_omicron = max_omicron[rep(1:nrow(max_omicron), numDays - max(max_omicron$day)),]
    # max_omicron$day = max_omicron$day[1] + 1:nrow(max_omicron)
    # omicronBQX_imports = bind_rows(plus_omicronBQX_imports, max_omicron)
    ########################################################################################################
    ...
    ## OmicronBQX
    init_cases_omicronBQX_plus_lines = sprintf('%.0f %.0f %.0f 7 1 %.0f', plus_omicronBQX_imports$day, plus_omicronBQX_imports$day, plus_omicronBQX_imports$Imports, plus_omicronBQX_imports$Imports)  
    init_cases_omicronBQX_normal_lines = sprintf('%.0f %.0f %.0f 7 1 %.0f', omicronBQX_imports$day, omicronBQX_imports$day, omicronBQX_imports$Imports, omicronBQX_imports$Imports)  
    
    init_cases_lines = c(init_cases_base_lines, init_cases_alpha_lines, init_cases_gamma_lines, init_cases_kappa_lines, init_cases_delta_lines, init_cases_omicron_lines, init_cases_omicronBAX_plus_lines, init_cases_omicronBQX_plus_lines)
    init_cases_plus_lines = c(init_cases_base_lines, init_cases_alpha_lines, init_cases_gamma_lines, init_cases_kappa_lines, init_cases_verylatedelta_lines, init_cases_omicron_lines, init_cases_omicronBAX_plus_lines, init_cases_omicronBQX_plus_lines)   
    init_cases_normal_lines = c(init_cases_base_lines, init_cases_alpha_lines, init_cases_gamma_lines, init_cases_kappa_lines, init_cases_verylatedelta_lines, init_cases_omicron_lines, init_cases_omicronBAX_normal_lines, init_cases_omicronBQX_normal_lines)   
...
intervention_df_default = intervention_df %>%    
  mutate(seed = floor(scalars_sobol_df$seed),
         ...
         variantomicronBQX_transmissibility =  scalars_sobol_df$variantomicronBQX_transmissibility,
         variantomicronBQX_transmissibility_factor =  scalars_sobol_df$variantomicronBQX_transmissibility_factor,
         variantomicronBQX_cross_protection_prob = scalars_sobol_df$variantomicronBQX_cross_protection_prob,
         variantomicronBQX_imports_factor = scalars_sobol_df$variantomicronBQX_imports_factor,
         variantomicronBQX_susceptibility_by_age_offset  = scalars_sobol_df$variantomicronBQX_susceptibility_by_age_offset,
         variantomicronBQX_susceptibility_by_age_rate = scalars_sobol_df$variantomicronBQX_susceptibility_by_age_rate,
         variantomicronBQX_susceptibility_by_age_cutoff = scalars_sobol_df$variantomicronBQX_susceptibility_by_age_cutoff,
         variantomicronBQX_susceptibility_by_age_high = scalars_sobol_df$variantomicronBQX_susceptibility_by_age_high,  
...
if(variants_in == 1){
  intervention_df_default$diseases = 8
  intervention_df_default$enable_disease_cross_protection = 1
  intervention_df_default$influenza_cross_protection_prob = 1.0
  intervention_df_default$disease_names = "8 influenza variantalpha variantgamma variantkappa variantdelta variantomicron variantomicronBAX variantomicronBQX"
}
  ...
  for(v in 1:length(vaccine_list)){
    ...
    normal_omicronBAX2_cross_050_transexcess_0_85_sev_060_omicron_cross_030_transexcess_0_5_severity_420_df =  intervention_tmp_df %>%
      mutate(intervention_id = sprintf("normal_omicronBAX2_cross_050_transexcess_0_85_sev_060_omicron_cross_030_transexcess_0_5_severity_420-vax_%d-mov_%s", vv, lockdown_list[ll]),
             ...
             variantomicronBQX_severity_factor = 0.3*variantomicron_severity_factor,
             variantomicronBQX_cross_protection_prob = 0.1,
             variantomicronBQX_transmissibility = 1.1*variantomicron_transmissibility,
             get_ifr_param_string(variantomicronBQX_severity_factor, 'omicronBQX')
      )  
    ...
    plus_omicronBAX2_cross_050_transexcess_0_85_sev_060_omicron_cross_030_transexcess_0_5_severity_420_df =  intervention_tmp_df %>%
      mutate(intervention_id = sprintf("plus_omicronBAX2_cross_050_transexcess_0_85_sev_060_omicron_cross_030_transexcess_0_5_severity_420-vax_%d-mov_%s", vv, lockdown_list[ll]),
             ...
             variantomicronBQX_severity_factor = 0.3*variantomicron_severity_factor,
             variantomicronBQX_cross_protection_prob = 0.2,
             variantomicronBQX_transmissibility = 1.2*variantomicron_transmissibility,
             get_ifr_param_string(variantomicronBQX_severity_factor, 'omicronBQX')               
      )  
    ...
    normal_omicronBAX2_cross_042_transexcess_0_85_sev_060_omicron_cross_030_transexcess_0_5_severity_420_df =  intervention_tmp_df %>%
      mutate(intervention_id = sprintf("normal_omicronBAX2_cross_042_transexcess_0_85_sev_060_omicron_cross_030_transexcess_0_5_severity_420-vax_%d-mov_%s", vv, lockdown_list[ll]),
             ...
             variantomicronBQX_severity_factor = 0.3*variantomicron_severity_factor,
             variantomicronBQX_cross_protection_prob = 0.3,
             variantomicronBQX_transmissibility = 2*variantomicron_transmissibility,
             get_ifr_param_string(variantomicronBQX_severity_factor, 'omicronBQX')
      ) 
...
scalars = scalars_intervention %>%
  mutate(days = numDays,
         ...
         variantomicronBQX_asymp_infectivity = asymp_infectivity_in,
         variantomicronBQX_susceptibility_by_age_minage = influenza_susceptibility_by_age_minage_in,
         variantomicronBQX_susceptibility_by_age_minvalue = influenza_susceptibility_by_age_minvalue_in,
         variantomicronBQX_face_mask_transmission_efficacy = face_mask_transmission_efficacy_in,
...
report_scalars = dplyr::select(
  ...
  variantomicronBQX_transmissibility,
  variantomicronBQX_transmissibility_factor,
  variantomicronBQX_cross_protection_prob, 
  variantomicronBQX_imports_factor,
  variantomicronBQX_severity_factor,
...
write_condor_sh_execution_file <- function(experiment_label_in, 
  ...
  sh_file_name <- paste0(output_path, "job_executor_", projection_label, ".sh")
...
get_running_jobs() {
    local WORKER=$1
    condor_status -claimed | grep -c $WORKER
}

# submit job to a specific worker
submit_job_to_worker() {
    local WORKER=$1
    sed -i "s/worker[0-9]\\+/$WORKER/g" %s
...
', worker_list, tmp_cmd_file, submission_file, submission_file, submission_file)
  
  script_file_name <- paste0(output_path, "condor_scheduler_", projection_label,".sh")
...
Transfer_Input_Files        = job_executor_STAGELABEL.sh, TMPCMDFILE, EXPERIMENTDIR, FREDHOMESTR
Transfer_Output_Files       = EXPERIMENTLABEL
...
"
    ...
    submission_str = submission_template %>%
        ...
        str_replace_all(pattern="STAGELABEL", replacement = projection_label) %>%
        str_replace_all(pattern="FREDHOMESTR", replacement = fred_home_dir_in) %>%
        str_replace_all(pattern="FREDRESULTSSTR", replacement = fred_results_in) %>%
        str_replace_all(pattern="JOBWALLTIME", replacement = walltime_in) %>%
