...
output.dir = file.path(scratch_dir, sprintf('FRED_%.0f_postcalibration_asymp_%.2f_fm_%.2f_ksus_%.2f_var_%.0f_vax_%03d_mov',state_code, asymp_infectivity_in, face_mask_transmission_efficacy_in, kids_susceptibility_age_in, variants_in, vaccination_in))
...
file.copy('../input_files/params_covid_omicron.txt','./input_files/params_covid_omicron.txt', overwrite = T)
...
#     prob_array = exp(-(params_sweep_ll$LL_deaths + params_sweep_ll$LL_dom*5)*particles_w)
# }
...
## omicron transmissibility: 0.5 - 1.5
## omicron  cross protection: 0.2 - 0.5

scalars_sobol_df$variantomicron_transmissibility_factor = 1.2
scalars_sobol_df$variantomicron_transmissibility = scalars_sobol_df$variantomicron_transmissibility_factor * scalars_sobol_df$variantdelta_transmissibility
scalars_sobol_df$variantomicron_cross_protection_prob = 0.5 
scalars_sobol_df$variantomicron_introduction_day = as.numeric(as.Date('2021-11-15') - as.Date('2020-01-01'))
scalars_sobol_df$variantomicron_severity_factor = scalars_sobol_df$variantdelta_severity_factor * 0.3
...
for(nn in 1:reps){
    ...
    if(variants_in == 1){
        ...
        tmp_df_variantomicron = tmp_df[tmp_df$Date >= as.Date('2020-01-01') + round(scalars_sobol_df$variantomicron_introduction_day[nn]) & tmp_df$Date <= as.Date('2022-02-01'),]
        
        init_cases_lines = sprintf('%.0f %.0f %.0f 0 1 %.0f', tmp_df_main$day, tmp_df_main$day, tmp_df_main$Imports, tmp_df_main$Imports)
        ...
        init_cases_omicron_lines = sprintf('%.0f %.0f %.0f 5 1 %.0f', tmp_df_variantomicron$day, tmp_df_variantomicron$day, tmp_df_variantomicron$Imports, tmp_df_variantomicron$Imports)        

        init_cases_lines = c(init_cases_lines, init_cases_alpha_lines,init_cases_kappa_lines,init_cases_gamma_lines, init_cases_delta_lines, init_cases_omicron_lines)
    }else{
...
shelter_high_mov_file = file.path(output.dir, sprintf('shelter_high_mov_timeseries_%s_%d.txt',state_code,1:reps))
...
for(nn in 1:reps){
    ...
    ## Temporary high mobility
    high_mov_date = as.Date('2021-04-30')
    ##high_mov_date = fit_date
    tmp_mov = tmp_df
    tmp_mov$shelter_trend[tmp_mov$date > high_mov_date] = tmp_mov$shelter_trend[tmp_mov$date > as.Date(fit_date)] * 0.0
    ...
    tmp_mov_tail = tmp_mov[tmp_mov$day == max_tmp_day,]
    tmp_mov_tail$day = max_tmp_day + 1
    tmp_mov_tail$shelter_trend = post_lockdown_mobility_in * tmp_mov_tail$shelter_trend
    ...
    tmp_mov$shelter_trend[tmp_mov$shelter_trend < 0.0001] = 0.0
    tmp_mov$shelter_trend[tmp_mov$shelter_trend > 1.0] = 1.0
    
    shelter_time_lines = sprintf('%.0f %.0f %.4f 0 120 %d%s', tmp_mov$day, tmp_mov$day, tmp_mov$shelter_trend, tmp_mov$State, tmp_mov$SCACODIGO)

    if(numDays > max(tmp_mov_tail$day)){
        shelter_time_tail_lines = sprintf('%.0f %.0f %.4f 0 120 %d%s', tmp_mov_tail$day, numDays, tmp_mov_tail$shelter_trend, tmp_mov_tail$State, tmp_mov_tail$SCACODIGO)
        shelter_time_lines = c(shelter_time_lines,shelter_time_tail_lines)
    }
    ## Add shelter by age
    ## shelter_age_lines = sprintf("82 212 %.4f 60 120", scalars_sobol_df$shelter_in_place_age_compliance[nn])
    fileConn<-file(shelter_high_mov_file[nn])
    writeLines(shelter_time_lines, fileConn)
    close(fileConn)
}
...
community_timeseries_mov_file = file.path(output.dir, sprintf("community_timeseries_mov_%d.txt", 1:reps))
...
for(nn in 1:reps){
    ...
    mov_comm_df = tmp_comm_df
    mov_comm_df$contact_rate[mov_comm_df$date >= as.Date('2021-05-07')] = mov_comm_df$contact_rate[mov_comm_df$date >= as.Date('2021-05-07')] * 1.1

    community_time_lines = sprintf('%.0f %.0f %.4f', tmp_comm_df$day, tmp_comm_df$day, tmp_comm_df$contact_rate)
    community_time_mov_lines = sprintf('%.0f %.0f %.4f', mov_comm_df$day, mov_comm_df$day, mov_comm_df$contact_rate)

    fileConn<-file(community_timeseries_file[nn])
    writeLines(community_time_lines, fileConn)
    close(fileConn)

    fileConn<-file(community_timeseries_mov_file[nn])
    writeLines(community_time_mov_lines, fileConn)
    close(fileConn)
}
...
vaccine_stock_df = vaccine_stock_df %>% drop_na()
...
## Write files
{
    ...
    ## School open at full capacity on June 8
    school_open_all_lines = c(   
        school_open_to_date_lines,
        sprintf("%d %d 1 17 1.0",  as.integer(as.Date('2021-06-08') - as.Date(start_date)) + 1, numDays),
        sprintf("%d %d 18 20 1.0",  as.integer(as.Date('2021-06-08') - as.Date(start_date)) + 1, numDays))
...
intervention_base = scalars_sobol_df %>%
    mutate(
...
intervention_df_default = intervention_df %>%    
    mutate(seed = floor(scalars_sobol_df$seed),
           ...
           variantomicron_introduction_day = scalars_sobol_df$variantomicron_introduction_day,
...
intervention_mov_df = intervention_df_default %>%
    mutate(intervention_id = "high_mov",
           shelter_in_place_file = shelter_high_mov_file)
...
intervention_vax_delta_low_df = intervention_df_default %>%
    mutate(intervention_id = "delta_low_vax",
           shelter_in_place_file = shelter_time_file,
           enable_vaccination = 1,
           variantdelta_cross_protection_prob = 0.8,
           )

intervention_mov_vax = intervention_mov_df %>%
    mutate(intervention_id = "high_mov_vax",
           shelter_in_place_file = shelter_high_mov_file,
           enable_vaccination = 1)


##scalars_intervention = bind_rows(intervention_df_default, intervention_mov_df, intervention_delta_df,
##                                 intervention_vax_df,intervention_mov_vax, intervention_delta_vax)

scalars_intervention = bind_rows(intervention_vax_df)
...
if(variants_in >= 1){
    ...
    system(sprintf("cat %s %s %s %s %s %s %s > %s",
                   defaults_covid_params, defaults_alpha_params, defaults_gamma_params,
                   defaults_kappa_params,defaults_delta_params, defaults_omicron_params, defaults_vaccine_params,
...
write_fred_parameters(scalars, defaults_params, output.dir,
                      basename.in = basename_params, fred_defaults = fred_defaults)
...
report_scalars = dplyr::select(
                            ...
                            variantdelta_severity_factor,
                            variantgamma_severity_factor,variantkappa_severity_factor,
                            variantomicron_transmissibility,variantomicron_transmissibility_factor,
                            variantomicron_cross_protection_prob, variantomicron_introduction_day,
                            ...
                            workplace_contacts, office_contacts,
                            workplace_contact_factor, household_contact_factor, household_contacts,
...
submit_jobs(experiment_supername_in = sprintf('FRED_TEST_POSTCAL_asymp_%.2f_FM_%.2f_KSUS_%.2f_V%.0f-VX-%d', asymp_infectivity_in, face_mask_transmission_efficacy_in, kids_susceptibility_age_in, variants_in, vaccination_in),
            ...
            FUN = write_cmd_function,cores_in=2,walltime_in = "6:00:00",
            subsys="SLURM",
