...
var_in = 1
vax_in = 70
outdir_st = file.path(outdir, sprintf('FRED_%d_projections_asymp_%.2f_fm_%.2f_ksus_%.2f_var_%.0f_vax_%03d_mov_out', ss, asymp_inf,fm_ef, ksus, var_in,vax_in))
...
#rt_symp_url = "https://saludata.saludcapital.gov.co/osb/wp-content/uploads/medios/Numero-de-Reproduccion-Efectivo-R(t)%20sintomaticos.csv"
#download.file(rt_url,'../epidata/Bogota_Reproduction_Number.csv')
#download.file(rt_symp_url,'../epidata/Bogota_Reproduction_Number_symp.csv')

#download.file("https://saludata.saludcapital.gov.co/osb/wp-content/uploads/medios/Numero-de-Reproduccion-Efectivo-Rt.csv", '../epidata/Bogota_Reproduction_Number.csv')
...
#data_symp_rt = read.delim('../epidata/Bogota_Reproduction_Number_symp.csv', fileEncoding  = "latin1" , skip = 5, sep = ";") #%>%
#    mutate(DateStart = parse_date_time(Fecha_Inicio_Ventana, orders = c("dmy", "dmy HMS")))%>%
#    mutate(Date = parse_date_time(Fecha_Fin_Ventana, orders = c("dmy", "dmy HMS")))%>%
#    mutate(R_mean = `Mean.R.`, R_std = `Std.R.`)%>%
#    mutate(R_mean = as.numeric(str_replace(R_mean, ",", "\\."))) %>%
...
param_names_fit = c('workplace_contact_factor', 'neighborhood_contact_factor', 'community_contact_rate_1', 'shelter_in_place_compliance', 'influenza_transmissibility',
                    'facemask_compliance',
                    'variantalpha_transmissibility_factor','variantalpha_cross_protection_prob',
                    'variantalpha_imports_factor','variantgamma_imports_factor', 'variantkappa_introduction_day',
                    'variantdelta_imports_factor',
                    'variantgamma_transmissibility_factor',
                    'variantgamma_cross_protection_prob', 'variantkappa_transmissibility_factor', 'variantkappa_cross_protection_prob','variantdelta_transmissibility_factor', 'variantdelta_cross_protection_prob','neighborhood_same_age_bias')
...
tmp_fred_all = state_fred %>%
    ...
    summarize(CF_median = quantile(CF_mean + CF_1_mean + CF_2_mean + CF_3_mean + CF_4_mean + CF_5_mean, probs = c(0.5), na.rm = T),
              CF_low = quantile(CF_mean + CF_1_mean + CF_2_mean + CF_3_mean + CF_4_mean + CF_5_mean, probs = c(0.025), na.rm = T),
              CF_high = quantile(CF_mean + CF_1_mean + CF_2_mean + CF_3_mean +  CF_4_mean + CF_5_mean, probs = c(0.975), na.rm = T),
              Cs_median = quantile(Cs_mean + Cs_1_mean + Cs_2_mean + Cs_3_mean + Cs_4_mean + Cs_5_mean, probs = c(0.5), na.rm = T),
              Cs_low = quantile(Cs_mean + Cs_1_mean + Cs_2_mean + Cs_3_mean + Cs_4_mean + Cs_5_mean, probs = c(0.025), na.rm = T),
              Cs_high = quantile(Cs_mean + Cs_1_mean + Cs_2_mean + Cs_3_mean + Cs_4_mean + Cs_5_mean, probs = c(0.975), na.rm = T),
              reinfections_median = quantile(reinfections / (C_1_mean + C_mean+ C_2_mean + C_3_mean + C_4_mean + C_5_mean), probs = c(0.5), na.rm = T),
              reinfections_low = quantile(reinfections / (C_1_mean + C_mean + C_2_mean + C_3_mean + C_4_mean + C_5_mean), probs = c(0.025), na.rm = T),
              reinfections_high = quantile(reinfections / (C_1_mean + C_mean + C_2_mean + C_3_mean + C_4_mean + C_5_mean), probs = c(0.975), na.rm = T),
              C_median = quantile(C_mean + C_1_mean + C_2_mean + C_3_mean + C_4_mean + C_5_mean, probs = c(0.5), na.rm = T),
              C_low = quantile(C_mean + C_1_mean + C_2_mean + C_3_mean + C_4_mean + C_5_mean, probs = c(0.025), na.rm = T),
              C_high = quantile(C_mean + C_1_mean+ C_2_mean + C_3_mean + C_4_mean + C_5_mean, probs = c(0.975), na.rm = T),
	      ...
	      DomOriginal_low = quantile(C_mean / (C_1_mean + C_mean + C_2_mean + C_3_mean + C_4_mean + C_5_mean), probs = c(0.025), na.rm = T),
	      DomOriginal_high = quantile(C_mean / (C_1_mean + C_mean + C_2_mean + C_3_mean + C_4_mean + C_5_mean), probs = c(0.975), na.rm = T),
              DomAlpha_median = quantile(C_1_mean / (C_1_mean + C_mean + C_2_mean + C_3_mean + C_4_mean + C_5_mean), probs = c(0.5), na.rm = T),
              DomAlpha_low = quantile(C_1_mean / (C_1_mean + C_mean + C_2_mean + C_3_mean + C_4_mean + C_5_mean), probs = c(0.025), na.rm = T),
              DomAlpha_high = quantile(C_1_mean /(C_1_mean + C_mean + C_2_mean + C_3_mean + C_4_mean + C_5_mean), probs = c(0.975), na.rm = T),
              DomGamma_median = quantile(C_2_mean / (C_1_mean + C_mean + C_2_mean + C_3_mean + C_4_mean + C_5_mean), probs = c(0.5), na.rm = T),
              DomGamma_low = quantile(C_2_mean / (C_1_mean + C_mean + C_2_mean + C_3_mean + C_4_mean + C_5_mean), probs = c(0.025), na.rm = T),
              DomGamma_high = quantile(C_2_mean /(C_1_mean + C_mean + C_2_mean + C_3_mean + C_4_mean + C_5_mean), probs = c(0.975), na.rm = T),
              DomKappa_median = quantile(C_3_mean / (C_1_mean + C_mean + C_2_mean + C_3_mean + C_4_mean + C_5_mean), probs = c(0.5), na.rm = T),
              DomKappa_low = quantile(C_3_mean / (C_1_mean + C_mean + C_2_mean + C_3_mean + C_4_mean + C_5_mean), probs = c(0.025), na.rm = T),
              DomKappa_high = quantile(C_3_mean /(C_1_mean + C_mean + C_2_mean + C_3_mean + C_4_mean + C_5_mean), probs = c(0.975), na.rm = T),
              DomDelta_median = quantile(C_4_mean / (C_1_mean + C_mean + C_2_mean + C_3_mean + C_4_mean + C_5_mean), probs = c(0.5), na.rm = T),
              DomDelta_low = quantile(C_4_mean / (C_1_mean + C_mean + C_2_mean + C_3_mean + C_4_mean + C_5_mean), probs = c(0.025), na.rm = T),
              DomDelta_high = quantile(C_4_mean /(C_1_mean + C_mean + C_2_mean + C_3_mean + C_4_mean + C_5_mean), probs = c(0.975), na.rm = T),
              DomOmicron_median = quantile(C_5_mean / (C_1_mean + C_mean + C_2_mean + C_3_mean + C_4_mean + C_5_mean), probs = c(0.5), na.rm = T),
              DomOmicron_low = quantile(C_5_mean / (C_1_mean + C_mean + C_2_mean + C_3_mean + C_4_mean + C_5_mean), probs = c(0.025), na.rm = T),
              DomOmicron_high = quantile(C_5_mean /(C_1_mean + C_mean + C_2_mean + C_3_mean + C_4_mean + C_5_mean), probs = c(0.975), na.rm = T),
              Phosp_median = 0,
              Phosp_low = 0,
              Phosp_high = 0,
              Chosp_median = quantile(Chosp_mean + Chosp_1_mean + Chosp_2_mean +  Chosp_3_mean  + Chosp_4_mean + Chosp_5_mean, probs = c(0.5), na.rm = T),
              Chosp_low = quantile(Chosp_mean + Chosp_1_mean + Chosp_2_mean + Chosp_3_mean + Chosp_4_mean + Chosp_5_mean, probs = c(0.025), na.rm = T),
              Chosp_high = quantile(Chosp_mean + Chosp_1_mean + Chosp_2_mean + Chosp_3_mean + Chosp_4_mean + Chosp_5_mean, probs = c(0.975), na.rm = T),
    ...
    mutate(RR_total = RR_median + RR_1_median + RR_2_median + RR_3_median + RR_4_median + RR_5_median,
           AR_median = 100 * (cumsum(C_0_median)) / N_median[1],
           AR_1_median = 100 * (cumsum(C_1_median)) / N_median[1],
           AR_2_median = 100 * (cumsum(C_2_median)) / N_median[1],
           AR_3_median = 100 * (cumsum(C_3_median)) / N_median[1],
           AR_4_median = 100 * (cumsum(C_4_median)) / N_median[1],
           AR_5_median = 100 * (cumsum(C_5_median)) / N_median[1])
...
write_csv(tmp_fred_all, '../output/fred_output_model_fit.csv')
