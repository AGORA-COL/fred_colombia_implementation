...
output.dir = file.path(scratch_dir, sprintf('FRED_%.0f_projections_asymp_%.2f_fm_%.2f_ksus_%.2f_var_%.0f_vax_%03d_mov_saved_lives_restriction',state_code, asymp_infectivity_in, face_mask_transmission_efficacy_in, kids_susceptibility_age_in, variants_in, vaccination_in))
...
for(nn in 1:reps){
    ...
    tmp_low_df$shelter_trend[tmp_df$date > as.Date('2020-01-01')] = 0.0
...
for(nn in 1:reps){
    ...
    high_comm_df$contact_rate[high_comm_df$date >= as.Date('2020-01-01')] = 1
...
facemask_compliance_off_file = file.path(output.dir, sprintf('facemask_compliance_off_%d_%d.txt',state_code,1:reps))
...
for(nn in 1:reps){
    ...
    off_day = as.Date('2020-01-01') - as.Date(start_date)
    tmp_off_df = tmp_df
...
school_schedule_no_restriction_all_file = file.path(output.dir, sprintf('school_schedule_no_restriction_all_%d.txt',state_code))
...
## Write files
{
    ...
    current_open_school_lines = c(current_open_school_lines, 
                                  sprintf("%d %d %d %d %.4f 0 11001%s",tmp_school_open$start_day,tmp_school_open$end_day, tmp_school_open$MinAge, tmp_school_open$MaxAge, tmp_school_open$Capacity, tmp_school_open$zipcode))
    current_open_school_lines = c(current_open_school_lines,
    ...
    school_open_to_date_lines = c(school_open_to_date_lines,
                                  sprintf("%d %d 18 20 %.4f",
                                          ...
                                          tmp_college_open_to_date$end_day, 
                                          tmp_college_open_to_date$Capacity))
    ...
    ## School open full capacity
    school_no_restriction_all_lines = c(
        sprintf("%d %d 1 17 1.0",  as.integer(as.Date('2020-01-01') - as.Date(start_date)) + 1, numDays),
        sprintf("%d %d 18 20 1.0",  as.integer(as.Date('2020-01-01') - as.Date(start_date)) + 1, numDays))
        
    fileConn<-file(school_schedule_no_restriction_all_file)
    writeLines(school_no_restriction_all_lines, fileConn)
    close(fileConn)
}
...
intervention_df_default = intervention_df %>%    
    mutate(seed = floor(scalars_sobol_df$seed),
           ...
           school_global_schedule_file = school_schedule_open_all_file,
...
intervention_vax_df = intervention_df_default %>%
    mutate(intervention_id = "default_vax_high_contacts",
           community_contact_timeseries_file = community_timeseries_high_file,
           shelter_in_place_file = shelter_time_file,
           enable_vaccination = 1
           )
...
for(ll in 1:length(lockdown_list)){
    ...
    for(v in 1:length(vaccine_list)){
        ...
        #####################################  Omicron   ############################################
        ############################################################################################# 
        add_omicron_high_contact =  intervention_tmp_df %>%
          mutate(intervention_id = sprintf("high_contact_plus_omicron-vax_%d-mov_%s", vv, lockdown_list[ll]),
                 community_contact_timeseries_file = community_timeseries_high_file,
                 shelter_in_place_file = shelter_time_lockdown_file,
                 face_mask_timeseries_file = facemask_compliance_off_file,
                 school_global_schedule_file = school_schedule_no_restriction_all_file,
                 ...
                 enable_vaccination = vv,
                 variantdelta_severity_factor = 1.378173828125,
        ...
        add_omicron_normal_contact =  intervention_tmp_df %>%
          mutate(intervention_id = sprintf("normal_contact_plus_omicron-vax_%d-mov_%s", vv, lockdown_list[ll]),
                 community_contact_timeseries_file = community_timeseries_file,
                 shelter_in_place_file = shelter_time_file,
                 face_mask_timeseries_file = facemask_compliance_file,
        ...
        scalars_intervention = bind_rows(scalars_intervention,
                                         add_omicron_high_contact,
                                         add_omicron_normal_contact
                                         )                                
    }
}
...
submit_jobs(experiment_supername_in = sprintf('FRED_SEV_PROJ_asymp_%.2f_FM_%.2f_KSUS_%.2f_V%.0f_VAX%d', asymp_infectivity_in, face_mask_transmission_efficacy_in, kids_susceptibility_age_in, variants_in, vaccination_in),
