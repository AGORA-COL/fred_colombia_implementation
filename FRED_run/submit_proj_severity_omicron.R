...
## Set STATE-------------------
##==============================================#
projection_label = 'severity'
state_code = 11001
reps = 7
reps_per_job = 1
forecast_date = "2022-04-30"
fit_date = "2022-04-18"
...
#         delta_imports = bind_rows(delta_imports, max_delta)
        ...
        init_cases_late_lines = c(init_cases_base_lines, init_cases_alpha_lines, init_cases_gamma_lines,init_cases_kappa_lines, init_cases_latedelta_lines, init_cases_omicron_lines)
...
for(ll in 1:length(lockdown_list)){
    ...
    for(v in 1:length(vaccine_list)){
        ...
        #####################################  Omicron   ############################################
        ############################################################################################# 
        omicroncross_070_transexcess_1_2_severity_28_df =  intervention_tmp_df %>%
          mutate(intervention_id = sprintf("omicron-cross-070-transexc-1_2-severity-28-vax_%d-mov_%s", vv, lockdown_list[ll]),
                 primary_cases_file = primary_cases_verylatedelta_omicron_file_in,
                 variantomicron_transmissibility = 1.2*variantdelta_transmissibility, ## Trans Excess 0.9, 1.2, 1.5, 2
                 variantomicron_cross_protection_prob = 0.7, ## Escape 0.5, 0.8 (Cross 0.5, 0.2)
                 enable_vaccination = vv,
                 vaccination_phases_enable_discrete_timing = 1,
                 variantdelta_severity_factor = 0.05*variantdelta_severity_factor,
                 variantomicron_severity_factor = 2.8*variantdelta_severity_factor,
                 get_ifr_param_string(variantomicron_severity_factor, 'omicron')
                 )
        # omicroncross_070_transexcess_1_2_severity_15_df =  intervention_tmp_df %>%
        #   mutate(intervention_id = sprintf("omicron-cross-070-transexc-1_2-severity-15-vax_%d-mov_%s", vv, lockdown_list[ll]),
        #          primary_cases_file = primary_cases_verylatedelta_omicron_file_in,
        #          variantomicron_transmissibility = 1.2*variantdelta_transmissibility, ## Trans Excess 0.9, 1.2, 1.5, 2
        #          variantomicron_cross_protection_prob = 0.7, ## Escape 0.5, 0.8 (Cross 0.5, 0.2)
        #          enable_vaccination = vv,
        #          vaccination_phases_enable_discrete_timing = 1,
        #          variantdelta_severity_factor = 0.05*variantdelta_severity_factor,
        #          variantomicron_severity_factor = 1.5*variantdelta_severity_factor,
        #          get_ifr_param_string(variantomicron_severity_factor, 'omicron')
        #          )
        # omicroncross_065_transexcess_1_2_severity_12_df =  intervention_tmp_df %>%
        #   mutate(intervention_id = sprintf("omicron-cross-065-transexc-1_2-severity-12-vax_%d-mov_%s", vv, lockdown_list[ll]),
        #          primary_cases_file = primary_cases_verylatedelta_omicron_file_in,
        #          variantomicron_transmissibility = 1.2*variantdelta_transmissibility, ## Trans Excess 0.9, 1.2, 1.5, 2
        ...
        #          vaccination_phases_enable_discrete_timing = 1,
        #          variantdelta_severity_factor = 0.05*variantdelta_severity_factor,
        #          variantomicron_severity_factor = 1.2*variantdelta_severity_factor,
        #          get_ifr_param_string(variantomicron_severity_factor, 'omicron')
        #          )
        # omicroncross_065_transexcess_1_2_severity_15_df =  intervention_tmp_df %>%
        #   mutate(intervention_id = sprintf("omicron-cross-065-transexc-1_2-severity-15-vax_%d-mov_%s", vv, lockdown_list[ll]),
        #          primary_cases_file = primary_cases_verylatedelta_omicron_file_in,
        #          variantomicron_transmissibility = 1.2*variantdelta_transmissibility, ## Trans Excess 0.9, 1.2, 1.5, 2
        #          variantomicron_cross_protection_prob = 0.65, ## Escape 0.5, 0.8 (Cross 0.5, 0.2)
        #          enable_vaccination = vv,
        ...
        scalars_intervention = bind_rows(scalars_intervention,
                                         omicroncross_070_transexcess_1_2_severity_28_df
                                         )                                
    }
}
...
print(scalars$variantgamma_prob_symptoms_values)
##===============================================##
