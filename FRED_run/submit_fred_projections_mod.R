...
output.dir = file.path(scratch_dir, sprintf('FRED_%.0f_projections_asymp_%.2f_fm_%.2f_ksus_%.2f_var_%.0f_vax_%03d_mov_scenarios',state_code, asymp_infectivity_in, face_mask_transmission_efficacy_in, kids_susceptibility_age_in, variants_in, vaccination_in))
...
for(nn in 1:reps){
    ...
    high_comm_df$contact_rate[high_comm_df$date >= as.Date('2022-04-15')] = high_comm_df$contact_rate[high_comm_df$date >= as.Date('2022-04-15')] * 1.1
    
    high_20_comm_df = tmp_comm_df
    high_20_comm_df$contact_rate[high_20_comm_df$date >= as.Date('2022-04-15')] = high_20_comm_df$contact_rate[high_20_comm_df$date >= as.Date('2022-04-15')] * 1.2
    
    high_40_comm_df = tmp_comm_df
    high_40_comm_df$contact_rate[high_40_comm_df$date >= as.Date('2022-04-15')] = high_40_comm_df$contact_rate[high_40_comm_df$date >= as.Date('2022-04-15')] * 1.4
    
    high_60_comm_df = tmp_comm_df
    high_60_comm_df$contact_rate[high_60_comm_df$date >= as.Date('2022-04-15')] = high_60_comm_df$contact_rate[high_60_comm_df$date >= as.Date('2022-04-15')] * 1.6
...
forecast_reopen_day = as.numeric(as.Date('2022-02-15') - as.Date(start_date))
...
community_file_list = list(
    ...
    community_timeseries_file,
    community_timeseries_high_40_file)
...
lockdown_list = c('base', 'all_schools_open', 'all_schools_open_40')
...
for(ll in 1:length(lockdown_list)){
    ...
    for(v in 1:length(vaccine_list)){
        ...
        omicroncross_065_transexcess_1_4_severity_01_df =  intervention_tmp_df %>%
          mutate(intervention_id = sprintf("omicron-cross-065-transexc-1_4-severity-01-vax_%d-mov_%s", vv, lockdown_list[ll]),
...
submit_jobs(experiment_supername_in = sprintf('FRED_SCENARIO_PROJ_asymp_%.2f_FM_%.2f_KSUS_%.2f_V%.0f_VAX%d', asymp_infectivity_in, face_mask_transmission_efficacy_in, kids_susceptibility_age_in, variants_in, vaccination_in),
