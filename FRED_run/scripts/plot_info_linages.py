...
### Simul settings ###
######################
forecast_day = dt.datetime(2023,6,30)
...
## Cases data
ut.FRED_cases().to_csv('../../data/FRED_cases.csv')

## Dom data
ut.FRED_dominance().to_csv('../../data/FRED_dominance.csv')
...
###############################
## Process simuls info
subprocess.run("Rscript ~/FRED_Implementation/fit_scripts/fit_post_process_lineages.R "+projection_label, shell=True, check=True)
...
#         ax[0,0].plot(date_days, deaths_5, alpha = 1, zorder = 1, color = colores[5], linewidth = 2)
    ...
    p8 = ax[0,0].plot(date_CF_fit, CF_6_fit_median,  alpha = 1, zorder = 2, color = colores[6], linewidth = 2)
    ax[0,0].fill_between(date_CF_fit, CF_6_fit_low, CF_6_fit_high, where=CF_6_fit_high >= CF_6_fit_low, facecolor=colores[6], interpolate=True, alpha = 0.1)
    ...
    ##### Dominance #####
    #####################
    mask = df_dom_data['variant'] == 'Alpha'
    ...
    mask = df_dom_data['variant'] == 'Gamma'
    ...
    mask = df_dom_data['variant'] == 'Delta'
    ...
    mask = df_dom_data['variant'] == 'Omicron'
    ...
    mask = df_dom_data['variant'] == 'BA.4 Omicron'
    ...
    omicronBAX_fit_low = df_fit[mask]['DomOmicronBAX_low'].to_numpy()
    omicronBAX_fit_high = df_fit[mask]['DomOmicronBAX_high'].to_numpy() 
    omicronBAX_fit_median = df_fit[mask]['DomOmicronBAX_median'].to_numpy()
    ...
    date_omicronBAX_fit = pd.to_datetime(df_fit[mask]['Date'].to_numpy(), format='%Y-%m-%d', errors='coerce')
    p5 = ax[0,1].plot(date_omicronBAX_fit, omicronBAX_fit_median,  alpha = 1, zorder = 2, color = colores[6], linewidth = 3)
    ax[0,1].fill_between(date_omicronBAX_fit, omicronBAX_fit_low, omicronBAX_fit_high, where=omicronBAX_fit_high >= omicronBAX_fit_low, facecolor=colores[6], interpolate=True, alpha = 0.1)
    ...
    AR_6_fit_low = df_fit[mask]['AR_6_low'].to_numpy()
    AR_6_fit_high = df_fit[mask]['AR_6_high'].to_numpy()
    AR_6_fit_median = df_fit[mask]['AR_6_median'].to_numpy()
    ...
    p8 = ax[1,1].plot(date_AR_fit, AR_6_fit_median,  alpha = 1, zorder = 2, color = colores[6], linewidth = 2)
    ax[1,1].fill_between(date_AR_fit, AR_6_fit_low, AR_6_fit_high, where=AR_6_fit_high >= AR_6_fit_low, facecolor=colores[6], interpolate=True, alpha = 0.1)
    ...
    Rt_6_median = df_fit[mask]['RR_6_median'].to_numpy()
    ...
    p8 = ax[1,0].plot(date_Rt_fit, Rt_6_median,  alpha = 0.4, zorder = 1, color = colores[6], linewidth = 2)
    ...
    p7 = ax[0,0].plot(pd.np.NaN, pd.np.NaN, color = colores[6], linewidth = 6)
    ...
    legend_label.append('BAX Omicron')
